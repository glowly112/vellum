...
9. **Library sheets.** `PaperSheet` should read as a page (cream/sage/ruled, catalog face on the paper), not a Notes row. No BOOK/HAND/TYPEWRITER chip. No loud TODAY/YESTERDAY section if the card already has a when. Empty desk: Fraunces **Empty** only — no second poetic line. Greeting should be Fraunces italic. Search + compose must still be system Liquid Glass. Not watched.

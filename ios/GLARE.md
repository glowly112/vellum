...
13. **GREETING_CLIP.** First paint of the library (empty and with pages): the full Fraunces italic greeting is visible — no slice by the status bar / Dynamic Island (phone 23). Stock large title; not a homemade draw; not a guessed 34pt pad. Extra air is `safeAreaPadding(.top)` (system default) so it does not hug the island (phone 24). A clear gap under the greeting so the date / first sheet / empty mark are not glued. Not watched.

...
final class HammerTests: XCTestCase {
    ...
    func testImportBackdatesAndSkipsDuplicates() {
        ...
        XCTAssertEqual(SettingsLook.versionLabel, "1.0.0 (32)")
        XCTAssertEqual(SettingsLook.buildNumber, "32")
        ...
        XCTAssertEqual(WelcomeLook.openingBeat, "sheet-then-letterpress")
        XCTAssertTrue(WelcomeLook.sheetArrives)
        XCTAssertTrue(WelcomeLook.sheetOwnsScreen)
        XCTAssertTrue(WelcomeLook.letterpress)
        XCTAssertTrue(WelcomeLook.letterpressHaptic)
        XCTAssertFalse(WelcomeLook.scalesStampUp)
        XCTAssertEqual(WelcomeLook.stampSide, 80)
        XCTAssertFalse(WelcomeLook.stampFillsMiddle)
        XCTAssertEqual(WelcomeLook.bounceResponse, DeskMotion.response)
        XCTAssertEqual(WelcomeLook.bounceDamping, DeskMotion.damping)
        XCTAssertEqual(WelcomeLook.bounceStartScale, 1.16)
        XCTAssertEqual(WelcomeLook.bounceSettle, 0.55)
        XCTAssertEqual(WelcomeLook.sheetSettle, 0.55)
        XCTAssertEqual(WelcomeLook.exitKind, "page-turn")
        XCTAssertEqual(WelcomeLook.skipExitKind, "spring-fade")
        XCTAssertFalse(WelcomeLook.exitIsCut)
        XCTAssertTrue(WelcomeChromeLook.contrasts(scheme: "light"))
        XCTAssertTrue(WelcomeChromeLook.contrasts(scheme: "dark"), "Dark Turn page is not cream-on-cream")
        XCTAssertEqual(WelcomeChromeLook.turnFillKind(scheme: "light"), "ink")
        XCTAssertEqual(WelcomeChromeLook.turnLabelKind(scheme: "light"), "paper")
        XCTAssertEqual(WelcomeChromeLook.turnFillKind(scheme: "dark"), "rust")
        XCTAssertEqual(WelcomeChromeLook.turnLabelKind(scheme: "dark"), "paper")
        XCTAssertFalse(WelcomeChromeLook.usesOnDeskFill)
        XCTAssertFalse(WelcomeChromeLook.creamOnCream)

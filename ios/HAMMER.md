...
| 5 | KB_COVER | No guessed pad (not 34 / 120). Word-count air only when keyboard lift > 0. Body is `text-editor`. Style last section is **Size**, detent `medium-first`, scroll pad ≥ 44. Keyboard-open gutters are **paper** (not system-white). Lift is `layout-guide`, not a jumped safe area — text travels with the keys. |

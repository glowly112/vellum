...
| 5 | Compose | System `square.and.pencil`, prompt **Search pages**. Fail a custom `+ New page` pill. Greeting is Fraunces italic on the stock large-title slot (system largeTitle size, not 34). Date · pages is Fraunces roman on the stock subtitle slot (subheadline, not SF, not a second headline). Extra air is `safeAreaPadding(.top)` with the system default — do not hug the island. Air under the greeting (`belowGreeting`) so the date line is not glued. First paint empty **and** with pages: full glyphs. Fail `GREETING_CLIP`. Do not hide the nav bar. |

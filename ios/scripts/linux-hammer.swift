...
enum LinuxHammer {
    static func main() {
        ...
        expect(EditorLook.hidesSystemScrollIndicator && EditorLook.boundEdgeKind == "path-ink", "E1 system chevron-pill is hidden")
        expect(BoundEdgeRailLook.kind == "path-ink" && BoundEdgeRailLook.placement == "trailing-edge", "E1 bound rail is a rust edge")
        expect(BoundEdgeRailLook.hidesSystemIndicator && !BoundEdgeRailLook.usesSystemFace, "E1 rail is path ink, not SF")
        expect(BoundEdgeRailLook.forbiddenFaces.contains("SF Pro") && BoundEdgeRailLook.forbiddenFaces.contains("Inter"), "E1 Inter and SF are forbidden on the rail")
        expect(BoundEdgeRailLook.hairlineInk == "rust" && BoundEdgeRailLook.thumbKind == "paper", "E1 rust hairline and cream paper thumb")
        expect(BoundEdgeRailLook.shortPageIsQuiet && !BoundEdgeRailLook.ownsScrolling, "E1 short page is quiet; TextEditor owns scroll")
        expect(!BoundEdgeRailLook.parksCaret && !BoundEdgeRailLook.usesScrollTo, "E1 rail does not park the caret")
        expect(!BoundEdgeRailLook.usesCaretRect && !BoundEdgeRailLook.capsBodyHeight, "E1 no caret-rect or height cap")
        expect(BoundEdgeRailLook.reduceMotionThumbIsInstant, "E1 reduce-motion thumb is instant")
        expect(BoundEdgeRailLook.restOpacity == 0 && BoundEdgeRailLook.hidesWhenIdle, "E1 rail is gone at rest")
        expect(BoundEdgeRailLook.shownHairlineOpacity > 0 && BoundEdgeRailLook.shownHairlineOpacity < 0.5, "E1 hairline is a wash, not full rust")
        expect(BoundEdgeRailLook.shownThumbFillOpacity > 0.8, "E1 thumb stays readable when shown")
        expect(BoundEdgeRailLook.idleHideSeconds == 1, "E1 rail hides after a second idle")
        expect(!BoundEdgeRailLook.reduceMotionFades, "E1 reduce-motion does not fade the rail")
        expect(!BoundEdgeRailLook.isLongPage(content: 400, bounds: 400), "E1 short page has no rail")
        expect(BoundEdgeRailLook.isLongPage(content: 900, bounds: 400), "E1 long page shows the rail")
        expect(BoundEdgeRailLook.progress(offset: 250, content: 900, bounds: 400) == 0.5, "E1 thumb tracks offset")
        expect(BoundEdgeRailLook.contentOffset(progress: 0.5, content: 900, bounds: 400) == 250, "E1 scrub writes offset only")
        ...
        expect(!ImportLook.hasSettingsScreen && ImportLook.livesInConnections, "import lives in Connections, not a settings dump")
        ...
        expect(ImportLook.hasSourceMarks && ImportLook.sourceMarkKind == "app-icon", "source marks are bundled app icons")
        expect(ImportMarkLook.kind == "app-icon" && !ImportMarkLook.usesDrawnPaths, "marks are not drawn pads")
        expect(!ImportMarkLook.usesSF && !ImportMarkLook.usesGenericCircle, "marks are not SF or generic circles")
        expect(ImportMarkLook.notes == "notes" && ImportMarkLook.journal == "journal" && ImportMarkLook.notion == "notion", "imagesets are notes, journal, notion")
        expect(ImportMarkLook.imagesets == ["notes", "journal", "notion"], "imageset names match the catalog")
        expect(ImportMarkLook.notes != "yellow-pad" && ImportMarkLook.journal != "brown-book" && ImportMarkLook.notion != "n", "homemade pad/book/N marks are gone")
        ...
        expect(SettingsLook.versionLabel == "1.0.0 (32)", "about version is 1.0.0 (32)")
        expect(SettingsLook.buildNumber == "32", "build number is 32")
        expect(SettingsLook.chromeFollowsColorScheme && SettingsChromeLook.followsColorScheme, "settings chrome follows colorScheme")
        expect(!SettingsChromeLook.usesCatalogIvory && !SettingsChromeLook.usesCatalogInk, "settings chrome is not catalog ivory/ink")
        expect(SettingsChromeLook.isNight(scheme: "dark") && SettingsChromeLook.darkFillHex == "1C1915", "Dark appearance → settings chrome is night")
        expect(SettingsChromeLook.isDay(scheme: "light"), "Light appearance → settings chrome is day desk")
        expect(!SettingsChromeLook.usesUIColorTraitCallback && SettingsChromeLook.resolver == "swiftui-colorScheme", "settings chrome is not UIColor traits")
        expect(SettingsChromeLook.darkRowHex == "27231E" && SettingsChromeLook.darkTypeHex == "F3EBDD", "night row and cream type")
        expect(SettingsChromeLook.resolvedScheme(appearanceRaw: "dark", system: "light") == "dark", "Dark override wins over light sheet traits")
        expect(!SettingsChromeLook.remapsCatalogPaper && !DeskLook.remapsCatalogPaper, "catalog paper/ivory stay cream")
        ...
        expect(!DeskSettings.lockDesk(in: defaults), "Face ID off by default")
        ...
        expect(WelcomeCopy.userFacing.allSatisfy { !WelcomeCopy.containsAppNameOrDesk($0) }, "welcome copy has no Velin/desk")
        ...
        expect(!WelcomeLook.libraryBehind, "no library behind welcome")
        expect(WelcomeLook.hasStamp && WelcomeLook.stampLetter == "V" && !WelcomeLook.showsAppName && !WelcomeLook.stampWritesName, "opening stamp is the paper V, not the app name")
        expect(WelcomeLook.openingBeat == "sheet-then-letterpress" && WelcomeLook.autoAdvanceAfterStamp, "sheet arrives, then letterpress, then Pages you keep")
        expect(WelcomeLook.sheetArrives && WelcomeLook.sheetOwnsScreen && WelcomeLook.letterpress, "opening is a cream sheet, then the stamp presses")
        expect(!WelcomeLook.scalesStampUp && WelcomeLook.stampSide == 80 && !WelcomeLook.stampFillsMiddle, "stamp stays the empty-library size, not a giant V")
        expect(WelcomeLook.bounceResponse == DeskMotion.response && WelcomeLook.bounceDamping == DeskMotion.damping, "letterpress is the delete spring")
        expect(WelcomeLook.bounceStartScale == 1.16 && WelcomeLook.letterpressHaptic, "stamp presses from proud, with a haptic")
        expect(WelcomeLook.sheetSettle == 0.55 && WelcomeLook.bounceSettle == 0.55, "sheet then stamp each settle")
        expect(WelcomeLook.exitKind == "page-turn" && WelcomeLook.skipExitKind == "spring-fade" && !WelcomeLook.exitIsCut, "Done turns into the library; Skip is not a cut")
        expect(WelcomeChromeLook.contrasts(scheme: "light") && WelcomeChromeLook.contrasts(scheme: "dark"), "Turn page contrasts in Light and Dark")
        expect(WelcomeChromeLook.turnFillKind(scheme: "light") == "ink" && WelcomeChromeLook.turnLabelKind(scheme: "light") == "paper", "Light Turn page is ink on cream")
        expect(WelcomeChromeLook.turnFillKind(scheme: "dark") == "rust" && WelcomeChromeLook.turnLabelKind(scheme: "dark") == "paper", "Dark Turn page is rust on cream, not cream-on-cream")
        expect(!WelcomeChromeLook.usesOnDeskFill && !WelcomeChromeLook.creamOnCream, "capsule fill is not onDesk")
        expect(WelcomeLook.cardsArrive && WelcomeLook.typesWriting, "cards arrive, writing types")
        expect(WelcomeLook.typeInterval == 0.045 && !WelcomeLook.typeShowsCursor, "typewriter is 45ms/char, no cursor")
        expect(WelcomeTypewriter.visible(full: "Sam", revealed: 2) == "Sa" && WelcomeTypewriter.visible(full: "Sam", revealed: 0) == "", "typewriter reveals a prefix")
        expect(WelcomeTypewriter.visible(full: "Sam", revealed: 8) == "Sam" && !WelcomeTypewriter.showsCursor, "typewriter finishes without a cursor")
        expect(WelcomeInkLook.sheetInk == "charcoal" && !WelcomeInkLook.sheetUsesPrimary && !WelcomeInkLook.sheetUsesOnDesk, "sheet writing is charcoal, not primary/onDesk")
        expect(!WelcomeInkLook.sheetInkFlipsWithScheme && WelcomeInkLook.headlineUsesOnDesk && WelcomeInkLook.creamSheetsStayCream, "sheet ink does not flip; headlines use onDesk")
        expect(AppearanceLook.retintsWholeApp && AppearanceLook.appliesPreferredColorSchemeAtRoot && AppearanceLook.appliesPreferredColorSchemeOnSheets, "one appearance tap retints the window and sheets")
        expect(!AppearanceLook.usesLocalSheetState && AppearanceLook.catalogSheetsStayCream, "tiles write AppStorage; catalog sheets stay cream")
        expect(!DeskLook.usesUIColorTraitCallback && DeskLook.resolver == "swiftui-colorScheme", "desk fill is not UIColor traits")

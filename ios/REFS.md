...
| Settings (`SettingsSheet`) | Bear General / Craft Appearance | `settings-chrome.jpg` | Inset grouped Form, section labels, close, toggles | Bear red, markdown toggles, Light/Dark that Notes-grays the desk |
| Connections | Craft Integrations | `connections-chrome.jpg` | Named sources in grouped rows | OAuth Connect to Linear |
| Welcome (`WelcomeView`) | Mesh paper-on-desk | `welcome-page-chrome.jpg` | Paper sheet on the desk, Skip / Done, page-turn | Mesh branding, sans, a 12-step account wizard |

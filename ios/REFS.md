...
| Editor paper | [Apple Notes blank ruled sheet](https://mobbin.com/screens/dc7e5d8a-f469-41cc-98c7-2aff7e58b84c) | `paper-notes.jpg` | Quiet floating chrome, keyboard-aware `TextEditor` | Full-bleed Notes page, or a condensed postcard / library card on the desk. Paper fills under the toolbar to the word-count `safeAreaInset`; grain peeks at the edges only |
...
| Title + body on paper | `vellum-editor.png` | Writing-surface charm: cream sheet, date, title, body. Native: that paper is the column (toolbar → word-count inset), not a floating card. `N words` / `Cream · Book` is a bottom `safeAreaInset` |
